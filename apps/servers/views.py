    ...
    def get(self, request):
        ...
        if search:
            ...
            try:
               ...
               pass
            servers = Server.objects.filter(Q(id=search_int) | Q(zctype__zctype__icontains=search) | Q(ipaddress__icontains=search)
    ...
    def get(self, request):
        ...
        if search:
            ...
            servers = Server.objects.filter(Q(zctype__zctype__icontains=search) | Q(ipaddress__icontains=search)
                                            | Q(description__icontains=search) | Q(brand__icontains=search)
                                            | Q(zcmodel__icontains=search) | Q(zcnumber__icontains=search)
                                            | Q(zcpz__icontains=search) | Q(owner__username__icontains=search)).order_by('zctype')
        else:
