...
class UserListView(LoginRequiredMixin, View):
    def get(self, request):
        ...
        if search:
            ...
            users = UserProfile.objects.filter(Q(username__icontains=search) | Q(staff_no__icontains=search) | Q(department__icontains=search) | Q(bg_telephone__icontains=search) | Q(mobile__icontains=search) | Q(email__icontains=search),is_superuser=0).order_by('-is_staff', 'staff_no')  # 排除超级管理员
        else:
